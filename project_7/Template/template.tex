...
All the Python solvers come up with the same $L_2$ norm (57.5457), which shows that the different methods are consistent with each other. On the other hand, the PETSc solver reports a different norm (51.7557) because it counts grid points differently: PETSc uses DMDA, which includes boundary points in its discretization, while the Python solvers focus only on the interior points.
