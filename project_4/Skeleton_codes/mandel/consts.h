...
Domain createDomain(Partition p) {
    ...
    long local_nx = IMAGE_WIDTH / p.nx;
    long local_ny = IMAGE_HEIGHT / p.ny;

    d.nx = local_nx;
    d.ny = local_ny;

    d.startx = p.x * local_nx;
    d.starty = p.y * local_ny;

    d.endx = d.startx + local_nx - 1;
    d.endy = d.starty + local_ny - 1;
